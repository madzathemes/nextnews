		...
		if(is_single()){
			...
			if (!empty( $next_post )) {
			 	if ( has_post_thumbnail( $next_post->ID ) ) {
					$custom_styles .='.nav-single .next { background-image: url("'.get_the_post_thumbnail_url( $size = 'post-thumbnail' ).'") }';
				}
		 	}
			$next_post = get_previous_post();
			if (!empty( $next_post )) {
			 	if ( has_post_thumbnail( $next_post->ID ) ) {
					$custom_styles .='.nav-single .previous { background-image: url("'.get_the_post_thumbnail_url( $size = 'post-thumbnail' ).'") }';
				}
		 	}
		}
