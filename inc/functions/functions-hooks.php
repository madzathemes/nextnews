...
function nextnews_css() {
	...
	if($p["top"] != "0px" or $p["bottom"] != "0px" or $p["left"] != "20px" or $p["right"] != "20px"){ $custom_styles .='.fixed-on  .mt-menu-bar.box .mt-bar-in,.fixed-on  .mt-menu-bar.ful .container { padding: '. esc_attr($p['top']) .' '. esc_attr($p['right']) .' '. esc_attr($p['bottom']) .' '. esc_attr($p['left']) .' } '; }
