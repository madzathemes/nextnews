<?php function nextnews_single_bottom() {
...
if ( false == get_theme_mod( 't_p_read_more', false ) ) { $t_p_read_more = esc_html__("Read More:", "nextnews");  } else { $t_p_read_more = get_theme_mod( 't_p_read_more' ); }
if ( false == get_theme_mod( 't_p_previous_article', false ) ) { $t_p_previous_article = esc_html__("Previous article", "nextnews");  } else { $t_p_previous_article = get_theme_mod( 't_p_previous_article' ); }
if ( false == get_theme_mod( 't_p_next_article', false ) ) { $t_p_next_article = esc_html__("Next article", "nextnews");  } else { $t_p_next_article = get_theme_mod( 't_p_next_article' ); }
if ( false == get_theme_mod( 't_p_may_be_intrested', false ) ) { $t_p_may_be_intrested = esc_html__("You may be interested", "nextnews");  } else { $t_p_may_be_intrested = get_theme_mod( 't_p_may_be_intrested' ); }
    ...
    <?php if ( get_the_author_meta( 'description' )) { ?>
      <div class="author-info">
        <div class="author-avatar">
          <?php echo get_avatar( get_the_author_meta( 'user_email' ), apply_filters( 'nextnews_author_bio_avatar_size', 55 ) ); ?>
          </div>
          <div class="author-description">
            <h5><?php echo esc_html__( 'Author', 'nextnews' ); ?></h5>
            <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author"><h3><?php printf( esc_html__( '%s', 'nextnews' ), get_the_author() ); ?></h3></a>
          ...
          <div class="author-link">
            <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
              <?php printf( esc_html__( 'View all posts by %s', 'nextnews' ), get_the_author() ); ?>
            </a>
          </div>
        </div>
      </div>
