 ...
 * @copyright 2017
 */
 if ( false == get_theme_mod( 't_c_comments_passowrd', false ) ) { $t_c_comments_passowrd = esc_html__("This post is password protected. Enter the password to view and post comments.", "nextnews");  } else { $t_c_comments_passowrd = get_theme_mod( 't_c_comments_passowrd' ); }
 if ( false == get_theme_mod( 't_c_name', false ) ) { $t_c_name = esc_html__("Name", "nextnews");  } else { $t_c_name = get_theme_mod( 't_c_name' ); }
 if ( false == get_theme_mod( 't_c_email', false ) ) { $t_c_email = esc_html__("Email", "nextnews");  } else { $t_c_email = get_theme_mod( 't_c_email' ); }
 if ( false == get_theme_mod( 't_c_website', false ) ) { $t_c_website = esc_html__("Website", "nextnews");  } else { $t_c_website = get_theme_mod( 't_c_website' ); }
 if ( false == get_theme_mod( 't_c_email_not_publish', false ) ) { $t_c_email_not_publish = esc_html__("Your email address will not be published.", "nextnews");  } else { $t_c_email_not_publish = get_theme_mod( 't_c_email_not_publish' ); }
 if ( false == get_theme_mod( 't_c_leave_a_comment', false ) ) { $t_c_leave_a_comment = esc_html__("Leave a Comment", "nextnews");  } else { $t_c_leave_a_comment = get_theme_mod( 't_c_leave_a_comment' ); }
 if ( false == get_theme_mod( 't_c_leave_a_reply_to', false ) ) { $t_c_leave_a_reply_to = esc_html__("Leave a Reply to %s", "nextnews");  } else { $t_c_leave_a_reply_to = get_theme_mod( 't_c_leave_a_reply_to' )." %s"; }
...
<?php endif; // check for comment navigation ?>
			...
			<ol class="commentlist">
				<?php
					wp_list_comments( array( 'callback' => 'nextnews_comment' ) );
				?>
			</ol>
...
if ( false == get_theme_mod( 't_c_you_must_be', false ) OR false == get_theme_mod( 't_c_logged_in', false ) OR false == get_theme_mod( 't_c_to_post_a_comment', false ) ) { $t_c_must_log_in = wp_kses(__( 'You must be <a href="%s">logged in</a> to post a comment.', 'nextnews' ), $nextnews_allowed_html_array );  } else { $t_c_you_must_be = get_theme_mod( 't_c_you_must_be' ); $t_c_logged_in = get_theme_mod( 't_c_logged_in' ); $t_c_to_post_a_comment = get_theme_mod( 't_c_to_post_a_comment' ); $t_c_must_log_in = $t_c_you_must_be .' <a href="%s">'.$t_c_logged_in.'</a> '.$t_c_to_post_a_comment;  }
if ( false == get_theme_mod( 't_c_loged_in_as', false ) OR false == get_theme_mod( 't_c_log_out', false ) ) { $t_c_loged_in_as = wp_kses(__( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account" class="mt-theme-text">Log out?</a>', 'nextnews' ), $nextnews_allowed_html_array );  } else { $t_c_loged_in_as = get_theme_mod( 't_c_loged_in_as' ); $t_c_log_out = get_theme_mod( 't_c_log_out' ); $t_c_log_out_of_this = get_theme_mod( 't_c_log_out_of_this' ); $t_c_loged_in_as = $t_c_loged_in_as.' <a href="%1$s">%2$s</a>. <a href="%3$s" title="'.$t_c_log_out_of_this.'" class="mt-theme-text">'.$t_c_log_out.'</a>';  }
